...
/// Constructs a minroot recursive snark
pub fn construct_recursive_snark(
    num_steps: usize,
    num_iters_per_step: usize,
) -> (
    PublicParams<Ep, Eq, MinRootCircuit<Fq>, TrivialTestCircuit<Fp>>,
    Vec<Fq>,
    Vec<Fp>,
    RecursiveSNARK<Ep, Eq, MinRootCircuit<Fq>, TrivialTestCircuit<Fp>>,
) {
    ...
    let start_recursive_snark_construction = Instant::now();
    ...
    println!(
        "RecursiveSNARK::construction took {:?}",
        start_recursive_snark_construction.elapsed()
    );

    assert!(recursive_snark.is_some());
    (pp, z0_primary, z0_secondary, recursive_snark.unwrap())
}

/// Verify the MinRoot RecursiveSNARK
pub fn verify(
    pp: &PublicParams<Ep, Eq, MinRootCircuit<Fq>, TrivialTestCircuit<Fp>>,
    recursive_snark: RecursiveSNARK<Ep, Eq, MinRootCircuit<Fq>, TrivialTestCircuit<Fp>>,
    z0_primary: Vec<Fq>,
    z0_secondary: Vec<Fp>,
    num_steps: usize,
) -> bool {
